  ...
  <a-layout-sider :trigger="null">
    ...
    <a-menu theme="dark" mode="inline" :defaultSelectedKeys="[menuActive]">
      ...
      <a-menu-item key="5" v-if="getAccountType === 'admin'">
        <router-link to="/view-companies">
          <a-icon type="bars" />
          <span>Companies List</span>
        </router-link>
      </a-menu-item>
      <a-menu-item key="6" v-if="getAccountType === 'admin'">
...
export default {
  created () {
    ...
    if (this.$route.fullPath === '/view-companies') this.menuActive = '5'
    if (this.$route.fullPath === '/add-company') this.menuActive = '6'
  },
  data () {
    return {
      menuActive: null
